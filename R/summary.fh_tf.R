...
summary.fh_tf <- function(object, ...) {
  ...
  # Normaly checks for residuals
  skewness_res <- skewness(object$model$resid, na.rm = TRUE)
  kurtosis_res <- kurtosis(object$model$resid, na.rm = TRUE)
  if (length(object$model$resid) >= 3 &&
    length(object$model$resid) < 5000) {
    shapiro_res_W <- shapiro.test(object$model$resid)[[1]]
    shapiro_res_p <- shapiro.test(object$model$resid)[[2]]
  } else {
