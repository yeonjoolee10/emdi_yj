...
fh_tf <- function(fixed, vardir, combined_data, domains, subdomains,
  ...
  if (MSE == FALSE){
    return(list(ind_Domain = point_estim$EBLUP_Area,
                ind_Subdomain = point_estim$EBLUP_SubArea,
                model = model,
                framework = fh_tf_fw,
    ...
    return(list(ind_Domain = point_estim$EBLUP_Area,
                ...
                MSE_Subdomain = mse_estim$MSE_SubArea,
                model = model,
                framework = fh_tf_fw,
                transformation = transformation,
                fixed = fixed,
...
mse_fh_tf <- function(fixed, vardir, domains, subdomains, nunits, trafo,
  ...
  # if(trafo == "log" | trafo == "arcsin"){
  #   bias_subarea_naive <- data.frame(SubArea = unique(framework$data[[subdomains]]))
  #   bias_area_naive <- data.frame(Area = unique(framework$data[[domains]]))
  # }
